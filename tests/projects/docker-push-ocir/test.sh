...
testDockerPushOcir () {
  testName=docker-push-ocir
  testDir=$testsDir/docker-push-ocir
  ...
  repo=iad.ocir.io/odx-pipelines/wercker-ocir-test/test-docker-push-ocir
  ...
  # now run the build pipeline - this creates an image with the specified repository setting, add --docker-local back latter
